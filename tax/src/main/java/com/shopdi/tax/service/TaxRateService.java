package com.shopdi.tax.service;

import com.shopdi.commonlibrary.exception.NotFoundException;
import com.shopdi.tax.constants.MessageCode;
import com.shopdi.tax.model.TaxRate;
import com.shopdi.tax.repository.TaxClassRepository;
import com.shopdi.tax.repository.TaxRateRepository;
import com.shopdi.tax.viewmodel.location.StateOrProvinceAndCountryGetNameVm;
import com.shopdi.tax.viewmodel.taxrate.TaxRateGetDetailVm;
import com.shopdi.tax.viewmodel.taxrate.TaxRateListGetVm;
import com.shopdi.tax.viewmodel.taxrate.TaxRatePostVm;
import com.shopdi.tax.viewmodel.taxrate.TaxRateVm;
...
public class TaxRateService {
    ...
    public TaxRateListGetVm getPageableTaxRates(int pageNo, int pageSize) {
        ...
        final List<Long> stateOrProvinceIds = taxRates.stream().distinct().map(el -> el.getStateOrProvinceId())
                .collect(Collectors.toList());

        if (!stateOrProvinceIds.isEmpty()) {
            // Call location service to get country names and state or province name by list
            // of state or province ids
            List<StateOrProvinceAndCountryGetNameVm> stateOrProvinceAndCountryGetNameVms = locationService
                    .getStateOrProvinceAndCountryNames(stateOrProvinceIds);
