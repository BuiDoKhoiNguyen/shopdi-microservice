package com.shopdi.order.specification;
...
import com.shopdi.order.utils.Constants;
...
public class OrderSpecification {
    ...
    public static Specification<Order> findMyOrders(String userId, String productName, OrderStatus orderStatus) {
        return (root, query, criteriaBuilder) -> {
            ...
            Predicate hasCreatedByPredicate = hasCreatedBy(userId).toPredicate(root, query, criteriaBuilder);
            Predicate hasOrderStatusPredicate = hasOrderStatus(orderStatus).toPredicate(root, query, criteriaBuilder);
            Predicate hasProductNamePredicate = hasProductNameInOrderItems(productName).toPredicate(root, query,
                    criteriaBuilder);
    ...
    public static Specification<Order> findOrderByWithMulCriteria(
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            Predicate withEmail = withEmail(email).toPredicate(root, query, criteriaBuilder);
            Predicate withOrderStatus = withOrderStatus(orderStatus).toPredicate(root, query, criteriaBuilder);
            Predicate withProductName = withProductName(productName).toPredicate(root, query, criteriaBuilder);
            Predicate withBillingPhoneNumber = withBillingPhoneNumber(billingPhoneNumber).toPredicate(root, query,
                    criteriaBuilder);
            Predicate withCountryName = withCountryName(countryName).toPredicate(root, query, criteriaBuilder);
            Predicate withDateRange = withDateRange(createdFrom, createdTo).toPredicate(root, query, criteriaBuilder);
    ...
    public static Specification<Order> hasCreatedBy(String createdBy) {
        return (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(Constants.Column.CREATE_BY_COLUMN),
                createdBy);
    }
            ...
            subquery.where(
                    criteriaBuilder.and(
                            ...
                            criteriaBuilder.like(
                                    criteriaBuilder
                                            .lower(orderItemRoot.get(Constants.Column.ORDER_ITEM_PRODUCT_NAME_COLUMN)),
                                    "%" + productName.toLowerCase() + "%")));
            return criteriaBuilder.exists(subquery);
