...
- After all above servers are running status, execute [deploy-shopdi-applications.sh](deploy-shopdi-applications.sh) file to deploy all of shopdi applications to `shopdi` namespace

```shell
./deploy-shopdi-applications
```

All of SHOPDI microservice deployed in `shopdi` namespace

- Setup hosts file
  edit host file `/etc/hots`

```shell
192.168.49.2 pgoperator.shopdi.local.com
192.168.49.2 pgadmin.shopdi.local.com
192.168.49.2 akhq.shopdi.local.com
192.168.49.2 kibana.shopdi.local.com
192.168.49.2 identity.shopdi.local.com
192.168.49.2 backoffice.shopdi.local.com
192.168.49.2 storefront.shopdi.local.com
192.168.49.2 grafana.shopdi.local.com
...
All configurations of SHOPDI application putted in the shopdi-configuration helm chart.

Bellow is the values of [values.yaml](../charts/shopdi-configuration/values.yaml)

## Yas helm charts

All charts of Yas application situated in `charts` folder

To Install the Yas helm charts access to [https://nashtech-garage.github.io/shopdi/](https://nashtech-garage.github.io/shopdi/)
